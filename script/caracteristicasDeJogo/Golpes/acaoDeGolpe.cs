...
public class acaoDeGolpe : MonoBehaviour {
	...
	void sobreVooAtivo()
	{
		...
		if(tempoDecorrido>ativa.tempoMoveMin && tempoDecorrido<ativa.tempoMoveMax)
		{
			...
			if(!addView)
			{ 
				adicionaOColisor(nomesGolpes.sobreVoo,"umCuboETrailMaior",ativa.tempoDestroy,"impactoDeVento");
				addView = true;
			}
...
//					print(procureUmBomAlvo(450));
	...
	void energiaDeGarrasAtiva()
	{
		...
		if(tempoDecorrido<ativa.tempoMoveMin)
		{
			...
			C = pegueOColisor(nomesGolpes.energiaDeGarras);
	...
	private void instanciaEletricidade(Vector3 paraOnde,float tempoMax = 10)
	{
		...
		proj.KXY = KXY;
		proj.criatureAlvo = procureUmBomAlvo(350);
	...
	void avanceEPareAbaixo(float distanciaDeParada = 1.75f)
	{
		...
		Transform T = procureUmBomAlvo();
		V = T!=null?T.position:V;
...
//		print(nomeColisor);
	...
	GameObject instancieEDestrua(string nomeGolpe,float tempoDeGolpe ,float tempoDeScript)
	{
